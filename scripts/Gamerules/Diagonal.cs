...
public class Diagonal : Object
{
	...
	public void CheckThreePointDiagonalAlignmentAndKillEnemies(Matrix matrix, Point p, Score score)
	{
		int row = p.GetX();
		int col = p.GetY();
	...
	{
		if (row <= rowLength - 3 && col <= colLength - 3)
		{
	...
	private void KillEnemies(Point[,] matrix, Point[] pts, int rowLength, int colLength, Score score)
	{
		...
		{
			FindAndKillEnemies(matrix, pts[2].GetX(), pts[2].GetY(), rowLength, colLength, 1, 1, ally, score);
		}
		else if (pts[2].GetX() == rowLength - 1 || pts[2].GetY() == colLength - 1)
		{
			FindAndKillEnemies(matrix, pts[0].GetX(), pts[0].GetY(), 0, 0, -1, -1, ally, score);
		}
		else
		{
			// Kill downward-right
